...
public interface SearchPhrController {
	...
	@ApiResponses(value = {
			@ApiResponse(code = 200, message = "Return PHR ADDRESS exist or not", response = Boolean.class),
			@ApiResponse(code = 422, message = "Invalid Request data, See API error for more details.", response = ApiError.class) })
	@ApiOperation(value = "Check the PHR Address exist or not.", response = Boolean.class)
	@ApiImplicitParam(value = "PHR Address", name = "phrAddress", required = true)
	ResponseEntity<Boolean> isPhrExists(@Valid String phrAddress);
